...
const MAX_SIGNAL_LENGTH = 192 * 1024
...
function checkRateLimit(socket: ServerWebSocket<ClientData>): void {
  const now = Date.now()
  ...
  if (socket.data.messagesInWindow > MAX_MESSAGES_PER_MINUTE) {
  ...
  return Bun.serve<ClientData>({
    ...
    fetch(request, server) {
      ...
      const upgraded = server.upgrade(request, {
        data: { rooms: new Set(), signals: new Map(), windowStartedAt: Date.now(), messagesInWindow: 0 },
      })
