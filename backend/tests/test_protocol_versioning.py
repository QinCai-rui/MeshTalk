...
class ApplyHandshakeTest(unittest.TestCase):
    ...
    def test_apply_rejects_v1_handshake(self):
        async def run():
            local = self._manager()
            remote = Identity.generate("Legacy")
            # A genuine v1 handshake (protocol_version=1), signed the legacy way.
            # With MIN_SUPPORTED_PROTOCOL_VERSION = 2 it must be rejected before
