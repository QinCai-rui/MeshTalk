...
function startupPhaseIndex(message: string): number {
  if (message.startsWith("Finalising")) return 3;
  if (message.startsWith("Loading")) return 2;
  if (message.startsWith("Connecting") || message.startsWith("Waiting")) return 1;
  return 0;
}

function StartupSplash({ message, width, height }: { message: string; width: number; height: number }) {
    ...
    >
      ...
      >
        ...
        {wide ? <box style={{ width: "100%", height: 7, flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
          ...
          <text fg="#526988">{burstFrame < STARTUP_BURST.length ? "INITIALIZING SECURE LINK" : "DIRECT  •  PRIVATE  •  TERMINAL-NATIVE"}</text>
          {!compact ? <text fg="#354867">◆──────────◆</text> : null}
        </box>}
        ...
        <box style={{ width: "100%", height: 2, flexDirection: "row", alignItems: "center", gap: 1 }}>
          <spinner name="simpleDotsScrolling" color="#65d6b4" />
