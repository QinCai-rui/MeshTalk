...
export function peerConnectionLabel(peer: Peer): string { if (!peer.is_online) return "Offline"; return peer.active_transport === "lan_tcp" ? "Direct (TCP)" : peer.active_transport === "remote_udp" ? "Direct (UDP)" : peer.active_transport === "remote_derp" ? "MeshTalk Relay" : "Offline" }
