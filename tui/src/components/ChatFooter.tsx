...
export function ChatFooter({ width, scrollFocused, status, openSettings }: { width: number; scrollFocused: boolean; status: string; openSettings: () => void }) {
  ...
  // Reserve the same area for hints and transient messages. Long notifications
  // clip within it, without moving the editor or showing a transient scrollbar.
  return <box height={width < 38 ? 5 : 3} flexShrink={0} paddingLeft={1} paddingRight={1} paddingTop={1}>
    {notification ? <box style={{ width: "100%", height: width < 38 ? 4 : 2, flexDirection: "row", alignItems: "flex-start", gap: 1 }}>
      <box id="chat-status" key={status} style={{ flexGrow: 1, flexShrink: 1, minWidth: 0, flexDirection: "column" }} overflow="hidden">
