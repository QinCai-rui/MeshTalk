...
export function ChatFooter({ width, scrollFocused, status }: { width: number; scrollFocused: boolean; status: string }) {
  ...
  const hint = compact
    ? [scrollFocused ? "Up/Down select / R reply / D delete" : "Enter send / Alt+Enter newline"]
    : [scrollFocused ? "Up/Down select / R reply / D delete / Enter image / End latest" : "Enter send / Alt+Enter newline / Ctrl+U attach / Ctrl+V paste image"]
  ...
  return <box height={width < 38 ? 5 : 3} flexShrink={0} paddingLeft={1} paddingRight={1} paddingTop={1}>
    ...
    </scrollbox> : <box style={{ width: "100%", flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", gap: 1 }}>
