...
function NotificationMenu({ dialogHeight, peers, selectedPeerId, identity, mutedPeers, showDialog }: Pick<Props, "dialogHeight" | "peers" | "selectedPeerId" | "identity" | "mutedPeers" | "showDialog">) {
  ...
  return <>{!peer && <text fg={theme.muted}>Select a peer in the sidebar first.</text>}{peer && peer.peer_id === identity?.peer_id && <text fg={theme.muted}>You cannot mute or unmute yourself.</text>}{peer && peer.peer_id !== identity?.peer_id && !peer.is_online && !isMuted && <text fg={theme.muted}>{peer.display_name} is not online.</text>}<MouseSelect focused height={Math.max(5, dialogHeight - 4)} options={[{ name: "Desktop alerts", description: "Delivery method, test alert, and alert types", value: "desktop" }, { name: "Selected peer alerts", description: selectedPeerId ? "Mute or unmute the selected peer" : "Select a peer first to manage their alerts", value: "peer" }, { name: "Back to Commands", description: "Return to Commands", value: "back" }]} onSelect={(_, option) => { if (option?.value === "desktop") showDialog({ kind: "notification-settings" }); else if (option?.value === "peer") showDialog({ kind: "notification-peer" }); else if (option?.value === "back") showDialog({ kind: "commands" }) }} wrapSelection showDescription /></>
}
