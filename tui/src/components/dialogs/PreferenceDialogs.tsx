...
export function ControlCustomDialogContent({ dialogHeight, dialogDraft, setDialogDraft, configureControl }: { dialogHeight: number; dialogDraft: string; setDialogDraft: (v: string) => void; configureControl: (url: string) => void }) {
  return (
    <SettingsScreen breadcrumb={["Connection", "Control server", "Custom server"]} description="Enter another secure WebSocket URL." dialogHeight={dialogHeight}>
...
export function ControlStatusDialogContent({ dialog, dialogHeight, showDialog }: { dialog: Extract<Dialog, { kind: "control-status" }>; dialogHeight: number; showDialog: (d: Dialog) => void }) {
  return (
    <SettingsScreen breadcrumb={["Connection", "Status"]} description="Current control server and discovery state." dialogHeight={dialogHeight}>
      ...
      ]} onSelect={(_, option) => { if (option?.value === "change") showDialog({ kind: "control" }) }} wrapSelection />
    </SettingsScreen>
  )
}
...
export function SplashStyleDialogContent({ splashStyle, dialogHeight, saveAdvancedConfig }: { splashStyle: SplashPreference; dialogHeight: number; saveAdvancedConfig: (p: Record<string, unknown>, m: string) => void }) {
  const current = splashStyle === "boot-log" ? "Boot log" : splashStyle === "card" ? "Animated card" : "Off"
  return <SettingsScreen breadcrumb={["Customisation", "Splash screen"]} description="Choose what appears during startup." dialogHeight={dialogHeight}>
    ...
    if (!option) return
    void saveAdvancedConfig({ splash_style: option.value }, `Splash screen set to ${option.value}.`)
    }} />
  </SettingsScreen>
}

export function ImageProtocolDialogContent({ dialog, dialogHeight, saveAdvancedConfig }: { dialog: Extract<Dialog, { kind: "advanced-image-protocol" }>; dialogHeight: number; saveAdvancedConfig: (p: Record<string, unknown>, m: string) => void }) {
  return <SettingsScreen breadcrumb={["Advanced", "Image protocol"]} description="Select the renderer MeshTalk uses for image attachments." dialogHeight={dialogHeight}>
    ...
    if (!option) return
    void saveAdvancedConfig({ image_protocol: option.value }, `Image protocol set to ${option.value}.`)
    }} />
  </SettingsScreen>
}
...
export function AdvancedControlIpDialogContent({ dialogHeight, dialogDraft, setDialogDraft, saveAdvancedConfig }: { dialogHeight: number; dialogDraft: string; setDialogDraft: (v: string) => void; saveAdvancedConfig: (p: Record<string, unknown>, m: string) => void }) {
  return (
    <SettingsScreen breadcrumb={["Advanced", "IP pinning", "Control server"]} description="Save control-server IP addresses directly." dialogHeight={dialogHeight}>
...
export function AdvancedStunIpDialogContent({ dialogHeight, dialogDraft, setDialogDraft, saveAdvancedConfig }: { dialogHeight: number; dialogDraft: string; setDialogDraft: (v: string) => void; saveAdvancedConfig: (p: Record<string, unknown>, m: string) => void }) {
  return (
    <SettingsScreen breadcrumb={["Advanced", "IP pinning", "STUN server"]} description="Save STUN-server IP addresses directly." dialogHeight={dialogHeight}>
