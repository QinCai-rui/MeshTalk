...
## Verification
<`Ran by workflow:` deterministic checks, then `Ran by reviewer:` exact agent-selected commands and results, then `Suggested (not run):` missing coverage. Never present an unrecorded check as if it passed.>
```
